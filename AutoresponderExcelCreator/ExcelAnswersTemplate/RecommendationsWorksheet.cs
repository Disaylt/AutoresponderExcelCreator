...
namespace AutoresponderExcelCreator.ExcelAnswersTemplate
{
    internal class RecommendationsWorksheet : ExcelWorksheetTemplate
    {
        ...
        {
            ("Купленный id","Наименование купленного товара","Рекомендуемый id","Рекомендуемое наименование"),
            ("В этой колнке необходимо записать купленный id","Здесь пишите как должен называться к ответе этот товар","(Может быть пустым) Если необходимо оставляете id товара, которые будете рекомендовать","В этой колонке пишите название товара, которй необходимо рекомендовать"),
